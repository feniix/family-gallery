...
interface PhotoCardProps {
  ...
  onPrivacyChange?: (mediaId: string, visibility: 'public' | 'family' | 'extended-family' | 'private') => void;
}
...
export function PhotoCard({ 
  ...
  }, [media.id, media.originalFilename, media.thumbnailPath, thumbnailUrl, thumbnailLoading, thumbnailError, priority, isIntersecting]);

  // Get current visibility from media metadata or default to 'family'
  const currentVisibility = (media as MediaMetadata & { visibility?: 'public' | 'family' | 'extended-family' | 'private' }).visibility || 'family';
  ...
  const handlePrivacyToggle = async (e: React.MouseEvent) => {
    ...
    try {
      const newVisibility = isPrivate ? 'family' : 'private';
      ...
      if (response.ok) {
        toast.success(
          newVisibility === 'private' 
            ? 'Photo marked as private' 
            : 'Photo made visible to family'
        );
        
        // Call the callback to update parent component
        if (onPrivacyChange) {
          onPrivacyChange(media.id, newVisibility);
        }
      } else {
        const errorData = await response.json();
        toast.error(errorData.error || 'Failed to update privacy');
      }
    } catch (error) {
  ...
  return (
    <div 
      ...
      className={`relative overflow-hidden rounded-lg bg-gray-100 cursor-pointer group ${
        aspectConfig.useInlineStyle ? '' : aspectConfig.className
      } ${isPrivate ? 'ring-2 ring-red-500 ring-opacity-50' : ''}`}
      style={aspectConfig.useInlineStyle ? aspectConfig.style : undefined}
      onClick={onClick}
    >
      ...
      {/* Privacy Status Indicator - Always visible for private content */}
      ...
      {/* Visibility indicator for non-private content (subtle) */}
      {currentVisibility !== 'family' && currentVisibility !== 'private' && (
        <div className="absolute top-2 left-2 bg-black bg-opacity-50 text-white rounded px-2 py-1 text-xs">
          {currentVisibility === 'public' ? '🌍' : currentVisibility === 'extended-family' ? '🏠' : ''}
        </div>
      )}

      {/* Privacy Toggle Button - Visible on hover when controls are enabled */}
      {showPrivacyControls && (
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <Button
            ...
            title={isPrivate ? "Make visible to family" : "Mark as private"}
          >
            {isUpdatingPrivacy ? (
